...
const UploadRtx: React.FC<FormInputProps> = ({ data }) => {
  const { control,setValue } = useFormContext<FormValues>();
  const [imageUrl,setImageUrl]=useState<any>(null);
  const [showError,setShowError]=useState(false);
useEffect(()=>{
  if (data.defaultValue!=undefined) {
    setImageUrl(import.meta.env.VITE_IMAGE_URL+ data.defaultValue)
  }
},[data])
   ...
   if (file) {
     if (!file.type.startsWith('image/')) {
      ...
      if (data.defaultValue) {
        ...
        setValue("deleteValue1",data?.defaultValue)
      }
     }
     return file;
   }
...
>
   ...
   <div className='w-[300px] h-[300px]'>
    {imageUrl?<img className='max-w-[300px] max-h-[300px]' src={imageUrl}/>:<>
  ...
  <p>(max, 800 X 800px)</p>
   ...
   {showError&&<AlertError data={
   { title:"Type Error",message:"Accebt Only Images"}
   }/>}
    </div>
  );
};

export default UploadRtx;
