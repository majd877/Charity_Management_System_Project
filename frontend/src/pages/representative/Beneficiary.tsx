...
const { data, isLoading, isError, refetch } = useGetQueryApiQuery({
  ...
  if (openDelete !== null) {
    return (
      <CreateDialog
        data={[]}
        title={`Delete Beneficiary ${deleteSelected}`}
        endPoint={`Beneficiary/delete/${openDelete}`}
  ...
  if (openEdit !== null) {
    return (
      <CreateDialog
        ...
        endPoint={`Beneficiary/update/${openEdit}`}
  ...
  return (
    <>
      <Tables
        ...
        endPoint="Beneficiary/get"
        title="المستفيدون"
       accessKeys={[
         { title: 'المستفيد',key:"image",is_image:true,name:"user.name",email:"user.email" },
