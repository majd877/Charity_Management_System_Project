...
const { data, isLoading, isError, refetch } = useGetQueryApiQuery({
  ...
  useEffect(()=>{
   if (!isLoading) {
     ...
     create[0].option=data.donors.map((item:{_id:string,name:string}) => ({ id: item._id, name: item.name }))
   }
  },[isLoading])
  if (isLoading) {
  ...
  if (openCreate) {
    return (
      <CreateDialog
        data={create}
        title="Create SendMessage"
        endPoint="SendMessage/create/donors"
  ...
  if (openDelete !== null) {
    return (
      <CreateDialog
        data={[]}
        title={`Delete SendMessage ${deleteSelected}`}
        endPoint={`SendMessage/delete/${openDelete}`}
  ...
  if (openEdit !== null) {
    return (
      <CreateDialog
        data={edit}
        title="Edit SendMessage"
        endPoint={`SendMessage/update/${openEdit}`}
  ...
  return (
    <>
      <Tables
        ...
        endPoint="SendMessage/get/donors"
        title="رسائل شكر للمتبرع"
        accessKeys={[
