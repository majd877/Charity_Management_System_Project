...
var deleteSelected = "";
const ComplaintsBeneficiaryUser = () => {
...
const { data, isLoading, isError, refetch } = useGetQueryApiQuery({
  ...
  if (openCreate) {
    return (
      <CreateDialog
        data={create}
        title="انشاء استفسارات المستفيدين"
        endPoint="ComplaintsBeneficiary/create"
  ...
  if (openDelete !== null) {
    return (
      <CreateDialog
        data={[]}
        title={`حذف استفسارات المستفيدين`}
        endPoint={`ComplaintsBeneficiary/delete/${openDelete}`}
  ...
  if (openEdit !== null) {
    return (
      <CreateDialog
        data={edit}
        title="تعديل استفسارات المستفيدين"
        endPoint={`ComplaintsBeneficiary/update/${openEdit}`}
  ...
  return (
    <>
      <Tables
        ...
        endPoint="ComplaintsBeneficiary/getByuser"
        title="استفسارات المستفيدين"
       accessKeys={[
         { title: 'المقدم',key:"image",is_image:true,name:"user_data.name",email:"" },
         { title: 'العنوان',key:"title",is_image:false,name:"name" },
         { title: 'الايميل',key:"user_data.email",is_image:false,name:"name" },
        ]}
      />
...
export default ComplaintsBeneficiaryUser;