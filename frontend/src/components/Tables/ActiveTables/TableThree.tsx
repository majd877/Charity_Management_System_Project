...
const TableThree = ({ data }: TableData) => {
          ...
          <tbody>
            {data.loading ? (
              ...
              data.data.map((packageItem, key) => (
                ...
                <tr className="hover:bg-gray-3 dark:hover:bg-meta-4" key={key}>
...
<div className="flex items-center space-x-3.5">
 {/* show */}
 {data.linkto? <button onClick={()=>{nav(`${data?.linkto}/${packageItem.id}`)}} className="hover:text-primary">
    <svg
  ...
  {/* delete */}
  <button onClick={()=>{data.handleDelete(packageItem)}}  className="hover:text-primary">
    <svg
      className="fill-current"
      width="18"
      height="18"
  ...
  {/* edit */}
  <button onClick={()=>{data.edit(packageItem);}} className="hover:text-primary">
